...
RSSA.Forecasting(L=19,groups=c(1:10),h=2,Y=datos_inflacion$INPC)


### Forecasting mas robusto a outliers ####
VSSA.Forecasting<-function(L,groups,h,Y){
  ...
  yhat<-array(U[1:L,groups],dim=c(L,length(groups)))%*%Lambda[groups,groups]%*%array(t(V[1:k,groups]),
                                                                                     dim=c(length(groups),k))
  pi<-array(U[L,groups],dim=length(groups))
  V2<-sum(pi^2)
...
length(datos_inflacion$INPC)/4
    
VSSA.Forecasting(L=41,groups=c(1:10),h=1,Y=datos_inflacion$INPC)
(abs(101.4737-96.094)/96.094)*100
